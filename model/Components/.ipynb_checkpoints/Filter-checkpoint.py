...
def meanFilter(img):
    ...
    mean_SDWI_plus_HAND = mean_SDWI_plus_HAND.set('system:time_start', img.get('system:time_start'))
    return mean_SDWI_plus_HAND
    
def medianFilter(img):
    median_SDWI = img.focal_median(radius=1,kernelType='square',units='pixels',iterations=1)
    median_SDWI = Resample(median_SDWI, 1)
    median_SDWI = median_SDWI.set('system:time_start', img.get('system:time_start'))
    return median_SDWI
